...
const Profile = () => {
	const [pageUser, setPageUser] = useState<DocumentData>();
	...
	const followBtn = () => {
		if (followState) {
			...
			handleUnfollow(currentUser.uid, profileId);
			console.log('Unfollow');
		} else {
			setFollowState(true);
			handleFollow(currentUser.uid, profileId);
			console.log('Follow');
		}
	};
	...
	const setUser = async () => {
		let profileData = await getUserData(profileId);
		if (profileData) setPageUser(profileData);
	};
	...
	const updatePosts = async () => {
		...
		//get posts data
		setPosts([]);
		const q = query(collection(db, 'posts'), where('id', 'in', pageUser.posts));
		...
		<main className={style.mainProfile}>
			...
			{popupOpen && (
				<div className={style.popupScreen}>
					<form className={style.popupForm} action="" onSubmit={(e) => handleSubmit(e)}>
						<input
							type="file"
							id="fileInput"
							accept="image/*"
							...
							required
						/>
						<SubmitBtn type="submit">Change Profile Photo</SubmitBtn>
